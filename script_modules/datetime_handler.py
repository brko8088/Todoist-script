...
def format_single_digit_to_two_digit(number):
    if number is None:
        return "00"
...
def get_time_in_clock_format(current_clock_time, shift_value):
    are_we_adding = False
    ...
    new_datetime = format_single_digit_to_two_digit(new_hours) + ":" + format_single_digit_to_two_digit(new_minutes)
    return str(new_datetime)


def get_datetime_in_due_string_format(previous_string, shift_value):
    if VERBOSE:
    ...
    for elem in string_date:
        if elem.find(":") > -1:
            string_date[x] = get_time_in_clock_format(elem, shift_value)
        x += 1
...
def get_datetime_in_ISO_format(date_time, shift_value):
    ...
    new_datetime = date_time[:11] + format_single_digit_to_two_digit(new_hours) + ":" + format_single_digit_to_two_digit(
                                new_minutes) + ":" + format_single_digit_to_two_digit(new_seconds) + "Z"
    return str(new_datetime)


def shift_schedule(token, task_item, shift_value):
    if task_item.due_string is not None:
        requests.post(
            ...
            data=json.dumps({
                ...
                "due_string": str(get_datetime_in_due_string_format(task_item.due_string, shift_value))
            }),
